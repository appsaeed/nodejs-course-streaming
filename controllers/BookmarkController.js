...
class BookmarkController extends Controller {
    ...
    static async index(req, res) {
        ...
        const get_bookmark = await (new Bookmark()).where('user_id', user_id).get();
        ...
        const playlists = await Promise.all(get_bookmark.map(async function (item) {
            const playlists = await (new Playlist()).where('id', item.playlist_id).get();
            return {
                ...item,
                playlists: playlists,
            }
        ...
        const bookmarks = await Promise.all(playlists.map(async function (item) {
            ...
            const items = await Promise.all(lists.map(async function (pl) {
                const tutor = await (new Tutor()).find(pl.tutor_id);
                return {
                    ...pl,
                    tutor: tutor
                }
